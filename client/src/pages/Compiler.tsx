...
export default function Compiler() {
  ...
  const run = useMutation({
    mutationFn: () => apiFetch<{ output: string; error: string | null }>("/compiler/run", {
      method: "POST",
  ...
  return (
    <Layout>
      ...
      <p className="text-muted-foreground mb-6">Monaco editor with live Python execution</p>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <GlassCard className="overflow-hidden">
          ...
          <Editor height="420px" defaultLanguage="python" theme="vs-dark" value={code} onChange={(v) => setCode(v || "")} />
        </GlassCard>

        <GlassCard className="p-4">
          <h2 className="font-semibold mb-2">Output</h2>
          <pre className="bg-muted/40 p-4 rounded-xl text-sm font-mono min-h-[200px] whitespace-pre-wrap">
            {error ? <span className="text-destructive">{error}</span> : output || "Run code to see output..."}
          </pre>
        </GlassCard>
      </div>
