...
export const InteractiveTerminal = forwardRef<InteractiveTerminalHandle, InteractiveTerminalProps>(
  function InteractiveTerminal(
    { code, height = 280, className, title = "Console", showToolbar = true, onStatusChange },
    ref
  ) {
        ...
        >
          {lines.length === 0 ? (
            <p className="text-muted-foreground text-xs">
              Press <strong className="text-foreground">Run in console</strong> (or ▶ above). When Python calls{" "}
              <code className="text-accent">input()</code>, type in the glowing line and press Enter.
            </p>
          ) : (
            lines.map((line, i) => (
